...
GPUS="${GPUS:-0,1,2}"
SEGMENT_END_UPDATE="${SEGMENT_END_UPDATE:-25}"
STAMP="${STAMP:-$(date +%Y%m%d_%H%M%S)}"
RUN_NAME="${RUN_NAME:-m1-qwen25-3b-calibrated-s0-u${SEGMENT_END_UPDATE}-${STAMP}}"
...
if (( ${#gpu_ids[@]} != 3 )); then
  echo "the registered recovery requires the original 3-GPU layout" >&2
  exit 2
fi
...
echo "LOGPROB_ALIGNMENT_PROFILE=qwen25_3b_calibrated"
echo "This is a production recovery fork; optimizer updates remain gated."
